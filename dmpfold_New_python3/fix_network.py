...
def calpha_to_main_chain(coords_ca):
    ...
    coord_ca_nterm = coords_ca[:, :1 , :] + 3.82 * F.normalize(torch.linalg.cross(vec_can2_can1, vec_can2_can3), dim=2)
    coord_ca_cterm = coords_ca[:, -1:, :] + 3.82 * F.normalize(torch.linalg.cross(vec_cac2_cac1, vec_cac2_cac3), dim=2)
    ...
    cross_vcan_vcac = F.normalize(torch.linalg.cross(vec_ca_can, vec_ca_cac, dim=2), dim=2)
...
class GRUResNet(nn.Module):
    ...
    def forward(self, x, x2, nloops=5, refine_steps=0):
        ...
        w, v = torch.linalg.eigh(M.float(), UPLO='U')
